...
- 修正本地模型评测缺失的危险样例召回率，并新增内容校验和绑定的 Adapter 发布清单；未通过重载、固定集、结构、安全和相对基线门禁的权重不能被服务加载。
- 新增脱敏实验结果发布器；Agent Lab 只根据校验和绑定的真实摘要显示 `verified_offline`，未生成摘要时显示 `not_trained`，不会根据推理 URL 猜测训练成功。
...
- 初次服务合并时全量后端测试为 605 passed；加入风险召回与发布门禁后为 609 passed；加入脱敏结果发布器与 API 白名单后为 612 passed，保留 2 条 Starlette 第三方弃用警告。
- Ruff 静态检查与格式检查：通过。
...
- 风险召回单元测试首次使用单一“胸痛”短语并错误假设其会触发高风险规则，导致安全合并断言失败；改为规则集已有的“胸痛并且呼吸困难”组合后重新验证，不为通过测试而修改规则。
