...
export function AlgorithmLabView() {
  ...
  return <div className="algorithm-view agent-lab">
    ...
    {intentEvaluation?.failure_taxonomy && <section className="algorithm-panel failure-taxonomy"><div className="challenge-title"><div><span className="algorithm-kicker">failure taxonomy · diagnostic only</span><h3>{isZh ? "错误分桶与路径救回" : "Failure taxonomy and path transitions"}</h3></div><strong>{isZh ? "禁止按测试集调参" : "no test-set tuning"}</strong></div><div className="transition-strip">{Object.entries(intentEvaluation.failure_taxonomy.transitions).map(([name, value]) => <article key={name}><small>{name}</small><b>+{value.rescued_from_rule}</b><span>{isZh ? "救回规则失败" : "rule failures rescued"}</span><em>-{value.regressed_from_rule} {isZh ? "回退" : "regressed"}</em></article>)}</div><div className="taxonomy-table"><div className="taxonomy-head"><span>{isZh ? "场景" : "Category"}</span><span>{isZh ? "规则 / 全量 / 混合" : "Rule / All / Hybrid"}</span><span>{isZh ? "主要失败字段" : "Dominant failure"}</span><span>{isZh ? "最佳观察路径" : "Best observed"}</span></div>{intentEvaluation.failure_taxonomy.categories.map((row) => <div className="taxonomy-row" key={row.category}><strong>{row.category}</strong><span>{pct(row.paths.rule_only.exact_pass_rate)} / {pct(row.paths.deepseek_all.exact_pass_rate)} / {pct(row.paths.hybrid.exact_pass_rate)}</span><code>{row.dominant_deepseek_failure || "—"}</code><span>{row.best_observed_paths.join(", ")}</span></div>)}</div><div className="evaluation-notes"><p><b>{isZh ? "下一批数据：" : "Next data: "}</b>{intentEvaluation.failure_taxonomy.next_data_contract.required_partition} · ≥{intentEvaluation.failure_taxonomy.next_data_contract.minimum_cases_per_priority_category}/{isZh ? "重点类别" : "priority category"}</p><p><b>{isZh ? "隔离要求：" : "Isolation: "}</b>must_not_copy_fixed_test_prompts=true</p></div></section>}

    {intentEvaluation?.development_protocol && <section className="algorithm-panel development-protocol"><div className="challenge-title"><div><span className="algorithm-kicker">independent development · protocol routing</span><h3>{isZh ? "独立开发集与字段路由" : "Independent development set and field routing"}</h3></div><strong>{intentEvaluation.development_protocol.dataset.cases} dev cases</strong></div><p className="muted">{isZh ? "这组样例只用于开发诊断，不进入训练，也不替代冻结测试集。低置信字段才升级到 DeepSeek，风险字段始终由确定性规则托底。" : "These cases are diagnostic-only and never enter training or replace the frozen test set. Only low-confidence fields escalate to DeepSeek; deterministic rules remain the risk authority."}</p><div className="transition-strip"><article><small>rule_only</small><b>{pct(intentEvaluation.development_protocol.paths.rule_only.exact_pass_rate)}</b><span>{isZh ? "精确通过" : "exact pass"}</span></article><article><small>rule_with_protocol</small><b>{pct(intentEvaluation.development_protocol.paths.rule_with_protocol.exact_pass_rate)}</b><span>{isZh ? "精确通过" : "exact pass"}</span></article><article><small>{isZh ? "测试集重合" : "test overlap"}</small><b>{intentEvaluation.development_protocol.isolation.exact_overlap_count}</b><span>max Jaccard {intentEvaluation.development_protocol.isolation.maximum_character_5gram_jaccard}</span></article></div><div className="evaluation-notes"><p><b>{isZh ? "路由阈值：" : "Routing thresholds: "}</b>primary ≥ {intentEvaluation.development_protocol.field_router.primary_intent_threshold} · secondary ≥ {intentEvaluation.development_protocol.field_router.secondary_intents_threshold}</p><p><b>{isZh ? "证据边界：" : "Evidence boundary: "}</b>human_reviewed=false · live_adapter_evaluated={String(intentEvaluation.development_protocol.field_router.evaluated_with_live_adapter)} · production_uplift=false</p><p><b>{isZh ? "当前结论：" : "Current finding: "}</b>{isZh ? "协议只小幅改善澄清字段，主意图、多意图和安全绕过仍需模型路由与后续独立测试验证。" : "The protocol improves clarification only modestly; primary intent, multi-intent, and safety bypasses still require model routing and an independent release test."}</p></div></section>}
