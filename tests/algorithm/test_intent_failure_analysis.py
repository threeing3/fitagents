...
def test_failure_analysis_aligns_paths_and_removes_prompts():
    report = analyze_failures(
        {
            "rule_only": [
                _row("a", "multi_intent", exact=False, failed="secondary_intents"),
                _row("b", "multi_intent", exact=True),
            ],
            "deepseek_all": [
                _row("a", "multi_intent", exact=True),
                _row("b", "multi_intent", exact=False, failed="clarification"),
            ],
            "hybrid": [
                _row("a", "multi_intent", exact=True),
                _row("b", "multi_intent", exact=True),
    ...
    assert report["transitions"]["deepseek_all"] == {
        "rescued_from_rule": 1,
        "regressed_from_rule": 1,
    }
