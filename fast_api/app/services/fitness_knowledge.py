...
class FitnessKnowledgeService:
    ...
    def retrieve_explanation_knowledge(
        ...
        if self.db is None:
            ...
            return [self._explanation_from_seed(item) for item in self._keyword_rank(items, query, top_k)]
        ...
        ranked = self._rank_model_items(candidates, query, "content")[:top_k]
        return [self._explanation_payload(item) for item in ranked]
    ...
    def retrieve_coaching_cases(
        ...
        if self.db is None:
            ...
            return [self._case_from_seed(item) for item in self._keyword_rank(items, query, top_k)]
        ...
        ranked = self._rank_model_items(candidates, query, "situation")[:top_k]
        return [self._case_payload(item) for item in ranked]
    ...
    def _rank_model_items(self, items: list[Any], query: str, text_attr: str) -> list[Any]:
        query_terms = self._terms(query)
        return sorted(
            items,
            key=lambda item: len(query_terms & self._terms(getattr(item, text_attr, ""))),
            reverse=True,
        )

    def _keyword_rank(self, items: list[dict[str, Any]], query: str, top_k: int) -> list[dict[str, Any]]:
        query_terms = self._terms(query)
        return sorted(
            items,
            key=lambda item: len(query_terms & self._terms(json.dumps(item, ensure_ascii=False))),
            reverse=True,
        )[:top_k]

    def _terms(self, text: str) -> set[str]:
        lowered = text.lower()
        terms = {lowered[index : index + 2] for index in range(max(len(lowered) - 1, 0))}
        terms.update(part for part in lowered.replace("/", " ").replace("_", " ").split() if part)
        return terms
    ...
    def _explanation_payload(self, item: models.ExplanationKnowledge) -> dict[str, Any]:
        return {
    ...
    def _explanation_from_seed(self, item: dict[str, Any]) -> dict[str, Any]:
        return {
            "knowledge_id": item["knowledge_id"],
            "topic": item["topic"],
            "content": item["content"],
    ...
    def _case_payload(self, item: models.CoachingCase) -> dict[str, Any]:
        return {
    ...
    def _case_from_seed(self, item: dict[str, Any]) -> dict[str, Any]:
        return {
            "case_id": item["case_id"],
            "case_type": item["case_type"],
